...
public class BlackBox {
	 ...
	 * 
	 * @param headerArray
	 *            A String Array containing header fields for the CSV file.
	 *            These will be added to the HashMap for the log
	 * @param match
	 *            A boolean that indicates if we're in a match or not True= FMS
	 *            Match and False= No FMS. Affects how the logfile is named.
	 * @param verbose
	 *            A boolean that
	 */
	public static void initializeLog(String[] headerArray, boolean match, boolean verbose) {
		...
		if (HEADER.get(0) != "Timestamp") {
			if (HEADER.get(0).toLowerCase().contains("time")) // Probably called
																// first field
																// something to
																// do with time,
																// so fix it
			{
				// User entered a timestamp for first field, but in a
				// non-standard form
				System.out.println("Next time call your first field 'Timestamp'");
				HEADER.set(0, "Timestamp"); // Set the first element to
											// "Timestamp"
			} else {
				// User probably didn't enter a timestamp field at all
				HEADER.add(0, "Timestamp"); // Add a "Timestamp" field at the
											// beginning
		 ...
		 * 
		 * The purpose of this section is to provide a human readable header for
		 * the log file
		 */
	...
	public static void writeLog() {
		...
		for (int i = 1; i < HEADER.size(); i++) {
			...
			if (value == null) // Checks to see if there's a null value in that
								// bucket
			{
				value = 0.0; // Set value to zero
				System.out.println("No value for '" + HEADER.get(i) + "'! Don't you want to log that?");
				logHash.put(HEADER.get(i), 0.0); // Put a value in there so we
													// only throw the above
													// error once
			}
	...
	private static void writeHeader() {
		...
		for (int i = 0; i < HEADER.size(); i++) {
			headerString = headerString + HEADER.get(i) + ","; // Compose a
																// header string

			if (!logHash.containsKey(HEADER.get(i))) // Check to see if we
														// already have a key in
														// our HashMap
			{
				logHash.put(HEADER.get(i), null); // Initialize keys for the
													// logHash
			}
		}
	...
	public static void logThis(String key, double value) {
		if (logHash.containsKey(key)) // Add the key and value to a HashMap if
										// key exists
		{
			...
			// Throw an error reminding to add to header upon initilization if
			// we have to add to HashMap
			System.out.println(
					"Logged Parameter '" + key + "' not found. Add this parameter to the Blackbox initilization.");
		}
	}
