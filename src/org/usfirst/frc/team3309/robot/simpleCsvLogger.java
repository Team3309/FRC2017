...
public class simpleCsvLogger {
	...
	String output_dir = "/U/"; // USB drive is mounted to /U on
												// roboRIO
	...
	/**
	 * Write a list of doubles to the output file, assuming it's open. Creates a
	 * new line in the .csv log file.
	 * 
	 * @param data_elements
	 *            Values to write (any number of doubles, each as its own
	 *            argument). Should have the same number of arguments here as
	 *            signal names/units set during the call to init()
	...
	/**
	 * Clears the buffer in memory and forces things to file. Generally a good
	 * idea to use this as infrequently as possible (because it increases
	 * logging overhead), but definitely use it before the roboRIO might crash
	 * without a proper call to the close() method (during brownout, maybe?)
	 * 
	...
	/**
	 * Closes the log file and ensures everything is written to disk. init()
	 * must be called again in order to write to a new file.
	 * 
